...
app.get('/api/products', (req, res) => {
    fs.readFile('dist/server/dataBase/products.json', (err, data) => {
        if (err) {
            console.log(err)
...
app.get('/api/basket', (req, res) => {
    fs.readFile('dist/server/dataBase/userBasket.json', (err, data) => {
        if (err) {
            console.log(err)
            res.send({ result: 0, text: err })
            return
...
app.post('/api/basket', (req, res) => {
    fs.readFile('dist/server/dataBase/userBasket.json', (err, data) => {
        ...
        fs.writeFile('dist/server/dataBase/userBasket.json', newBasket, (err) => {
            if (err) {
                console.log(err)
                res.send({ result: 0, text: err })
                return
